...
<div class="overflow-x-auto relative shadow-md sm:rounded-lg lg:m-32">
    ...
    <div class="lg:flex lg:justify-start sm:block">
        ...
        <div class="ml-2 ">
            ...
            <button type="submit" class="focus:outline-none text-white bg-green-500 hover:bg-green-900 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Verified Users</button>
            </form>
    </div>
        <div class="ml-2">
            <form action="{{ route('user.nonverified') }}" method="GET" role="search" >
            <button type="submit" class="focus:outline-none text-white bg-green-500 hover:bg-green-900 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Not Verified Users</button>
            </form>
        </div>
            <div class="ml-2">
                <form action="{{ route('user.admin') }}" method="GET" role="search" >
                <button type="submit" class="focus:outline-none text-white bg-green-500 hover:bg-green-900 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Admin Users</button>
                </form>
            </div>
                <div class="ml-2">
                    <form action="{{ route('user.default') }}" method="GET" role="search" >

                    <button type="submit" class="focus:outline-none text-white bg-green-500 hover:bg-green-900 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Default Users</button>
                    </form>
                </div>
      </div>
  ...
  <td class="py-4 px-6 overflow-clip">
                ...
                <td class="py-4 px-6 overflow-clip uppercase">
                    @if ($u->verified!=1)
                    <button type="button" data-modal-toggle="popup-modal{{$u->id}}" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Verify</button>
                    @else
                    <button type="button" data-modal-toggle="popup-modali{{$u->id}}" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Un-Verify</button>

                    @endif
                </td>
                <td class="py-4 px-6 overflow-clip uppercase">
                    @if($u->role!=1)
                    <button type="button" data-modal-toggle="popup-modalAdmin{{$u->id}}" class="focus:outline-none text-white bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:ring-indigo-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Make Admin</button>
                    @else
                    <button type="button" data-modal-toggle="popup-modaliUser{{$u->id}}" class="focus:outline-none text-white bg-yellow-700 hover:bg-yellow-800 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Make User</button>
                    @endif
                </td>
        ...
        <div class="relative bg-white rounded-lg shadow ">
            ...
            <div class="p-6 text-center">
              <form action="{{route('user.cverifiko',$u->id)}}" method="post">
                  @csrf
                <svg aria-hidden="true" class="mx-auto mb-4 w-14 h-14 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                <h3 class="mb-5 text-lg font-normal text-gray-500">Are you sure you want to un-verify this user?</h3>
